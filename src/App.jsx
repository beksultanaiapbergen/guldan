...
const FRESH = [
  {label:"Только срезаны", color:"#4ade80"},
  {label:"Вчера срезаны", color:"#fbbf24"},
  {label:"2–3 дня назад", color:"#f87171"},
];
const FLOWER_EMOJI = ["🌸","🌺","🌼","🌻","🌹","💐","🪷"];
  ...
  return (
    <div style={{position:"fixed",inset:0,overflow:"hidden",pointerEvents:"none",zIndex:0}}>
      {FLOWER_EMOJI.map((e,i)=>(
        <div key={i} style={{position:"absolute",fontSize:18+i*3,opacity:0.03+i*0.006,top:`${10+i*13}%`,left:`${5+i*14}%`,transform:`rotate(${i*27}deg)`,filter:"blur(0.5px)",animation:`drift${i} ${12+i*3}s ease-in-out infinite`}}>{e}</div>
      ))}
    </div>
  );
}
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    return onSnapshot(q,snap=>setPending(snap.docs.map(d=>({...d.data(),id:d.id}))));
  },[]);
  ...
  const addPending = async l => {
    ...
    sendTelegram(
      ...
      `🌿 Свежесть: ${FRESH[l.freshness]?.label||""}\n\n` +
      `✅ Зайдите в админку\n🔑 Код: 8775`
    );
  };
  ...
  const approve = async id => {
    ...
    if(!item) return;
    await addDoc(collection(db,"listings"),{...item,createdAt:serverTimestamp()});
    await deleteDoc(doc(db,"pending",id));
    sendTelegram(`✅ Объявление опубликовано: *${item.title||"Букет"}* — ${item.price} ₸`);
  };

  const reject = async id => {
    const item = pending.find(x=>x.id===id);
    await deleteDoc(doc(db,"pending",id));
    if(item) sendTelegram(`❌ Заявка отклонена: *${item.title||"Букет"}*`);
  };

  const removeL = async id => { await deleteDoc(doc(db,"listings",id)); };

  if(mode==="buy") return <BuyerView listings={listings} back={()=>setMode(null)}/>;
  if(mode==="sell") return <SellerView back={()=>setMode(null)} onPublish={l=>{addPending(l);setMode("sent");}}/>;
  if(mode==="sent") return <SentScreen back={()=>setMode(null)}/>;
  if(mode==="admin") return <AdminPanel pending={pending} listings={listings} approve={approve} reject={reject} removeL={removeL} back={()=>setMode(null)}/>;
  if(mode==="chats"&&chatSeller) return <ChatList sellerPhone={chatSeller.phone} sellerName={chatSeller.name} onOpen={(c)=>{
    const listing={id:c.listingId,title:c.listingTitle,price:c.listingPrice,sellerName:c.sellerName,sellerPhone:c.sellerPhone,city:""};
    setChatSeller(cs=>({...cs,openChat:{listing,buyerName:c.buyerName}}));
  }} onBack={()=>{setMode(null);setChatSeller(null);}}/>;
  if(chatSeller?.openChat) return <ChatRoom listing={chatSeller.openChat.listing} buyerName={chatSeller.openChat.buyerName} role="seller" onBack={()=>setChatSeller(cs=>({...cs,openChat:null}))}/>;

  if(showAdmin) return (
    <Shell>
      <div style={C.centerBox}>
        <div style={{fontSize:48}}>🔐</div>
        <h2 style={C.h2}>Админ панель</h2>
        <input style={{...C.inp,textAlign:"center",letterSpacing:10,fontSize:22}} type="password" placeholder="••••" maxLength={4}
          ...
          onKeyDown={e=>{if(e.key==="Enter"){if(adminCode===ADMIN_CODE){setMode("admin");setShowAdmin(false);setAdminCode("");}else setAdminErr(true);}}}/>
        {adminErr&&<p style={{color:"#f87171",fontSize:13,margin:0}}>Неверный код</p>}
        <button style={C.btn} className="gbtn" onClick={()=>{if(adminCode===ADMIN_CODE){setMode("admin");setShowAdmin(false);setAdminCode("");}else setAdminErr(true);}}>Войти</button>
      ...
      <div style={{...C.home,opacity:vis?1:0,transform:vis?"none":"translateY(28px)",transition:"all 0.7s cubic-bezier(.22,1,.36,1)"}}>
        ...
        <div style={C.modeRow}>
          <button style={{...C.modeCard,...C.modeBuy}} onClick={()=>setMode("buy")}>
            ...
            <span style={C.modeNum}>{listings.length} букетов</span>
        ...
        <button style={C.chatsEntry} onClick={()=>{
          const phone=prompt("Введите ваш номер телефона продавца:");
          if(phone){setChatSeller({phone,name:phone});setMode("chats");}
        }}>
...
function BuyerView({listings,back}) {
  ...
  const list=listings
    ...
    .sort((a,b)=>sort==="asc"?+a.price?.replace(/\s/g,"")-+b.price?.replace(/\s/g,""):sort==="desc"?+b.price?.replace(/\s/g,"")-+a.price?.replace(/\s/g,""):0);
  return (
    <Shell>
      <div style={C.topBar}>
        <button style={C.iconBtn} onClick={back}><svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7"/></svg></button>
        <span style={C.topTitle}>Каталог букетов</span>
        <button style={{...C.iconBtn,color:showF?"#fbbf24":"rgba(255,255,255,0.5)"}} onClick={()=>setShowF(v=>!v)}><svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M22 3H2l8 9.46V19l4 2v-8.54z"/></svg></button>
      </div>
      <div style={C.page}>
        ...
        {showF&&(
          <div style={C.filterPanel}>
            <div style={C.filterGroup}>
              ...
              <div style={C.pillRow}><Pill active={!city} onClick={()=>setCity("")}>Все</Pill>{CITIES.map(c=><Pill key={c} active={city===c} onClick={()=>setCity(c)}>{c}</Pill>)}</div>
            </div>
            <div style={C.filterGroup}>
              <span style={C.filterLbl}>СОРТИРОВКА</span>
              <div style={C.pillRow}>{[["new","Новые"],["asc","Дешевле"],["desc","Дороже"]].map(([v,l])=><Pill key={v} active={sort===v} onClick={()=>setSort(v)}>{l}</Pill>)}</div>
            </div>
          </div>
        )}
        ...
        {list.length===0
          ...
          : <div style={C.grid}>{list.map(l=><Card key={l.id} l={l} onClick={()=>setDetail(l)}/>)}</div>
        }
      </div>
    </Shell>
  );
...
function Card({l,onClick}) {
  ...
  const bgs=["linear-gradient(135deg,#fce4ec,#f8bbd0)","linear-gradient(135deg,#fff3e0,#ffe0b2)","linear-gradient(135deg,#e8f5e9,#c8e6c9)","linear-gradient(135deg,#e3f2fd,#bbdefb)","linear-gradient(135deg,#f3e5f5,#e1bee7)","linear-gradient(135deg,#fafafa,#f5f5f5)"];
  const idx=typeof l.id==="string"?l.id.charCodeAt(0)%bgs.length:l.id%bgs.length;
  return (
    <div style={C.card} className="card" onClick={onClick}>
      <div style={{...C.cardImg,background:l.photo?"none":bgs[idx]}}>
        {l.photo?<img src={l.photo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<span style={C.cardEmoji}>{FLOWER_EMOJI[idx%FLOWER_EMOJI.length]}</span>}
...
function Detail({listing:l,back}) {
  ...
  const [liked,setLiked]=useState(false);
  const fr=FRESH[l.freshness]||FRESH[0];
  const bgs=["linear-gradient(160deg,#fce4ec,#f8bbd0)","linear-gradient(160deg,#fff3e0,#ffe0b2)","linear-gradient(160deg,#e8f5e9,#c8e6c9)","linear-gradient(160deg,#e3f2fd,#bbdefb)","linear-gradient(160deg,#f3e5f5,#e1bee7)","linear-gradient(160deg,#fafafa,#f5f5f5)"];
  const idx=typeof l.id==="string"?l.id.charCodeAt(0)%bgs.length:l.id%bgs.length;

  if(chat) return <ChatRoom listing={l} role="buyer" onBack={()=>setChat(false)}/>;

  return (
    <Shell>
      <div style={{...C.detailHero,background:l.photo?"none":bgs[idx]}}>
        {l.photo?<img src={l.photo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<span style={C.detailEmoji}>{FLOWER_EMOJI[idx%FLOWER_EMOJI.length]}</span>}
        <div style={C.detailOverlay}/>
        <button style={C.backFloat} onClick={back}><svg width="18" height="18" fill="none" stroke="white" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7"/></svg></button>
        <button style={{...C.likeFloat,color:liked?"#f43f5e":"rgba(255,255,255,0.8)"}} onClick={()=>setLiked(v=>!v)}>{liked?"♥":"♡"}</button>
        <div style={C.priceBadge}>{l.price} ₸</div>
      </div>
      <div style={C.detailBody}>
        <h1 style={C.detailTitle}>{l.title||"Букет"}</h1>
        <div style={C.metaRow}>
          <span style={{...C.metaPill,background:fr.color+"18",color:fr.color,border:`1px solid ${fr.color}30`}}><div style={{...C.freshDot,background:fr.color}}/>{fr.label}</span>
        ...
        {l.flowers?.length>0&&<div style={C.section}><div style={C.sectionLbl}>СОСТАВ</div><div style={C.pillRow}>{l.flowers.map(f=><span key={f} style={C.metaPillGray}>{f}</span>)}</div></div>}
        {l.description&&<div style={C.section}><div style={C.sectionLbl}>О БУКЕТЕ</div><p style={C.desc}>{l.description}</p></div>}
        <div style={C.sellerRow}>
          <div style={C.sellerAva}>{l.sellerName?.[0]||"?"}</div>
          <div style={{flex:1}}><div style={C.sellerNm}>{l.sellerName}</div><div style={C.sellerRole}>Продавец · ⭐ 4.9</div></div>
        </div>
        <div style={C.contactBox}>
          {l.showPhone==="show"&&<a href={`tel:+${l.sellerPhone}`} style={{...C.callBtn,marginBottom:12}}>📞 Позвонить продавцу</a>}
          <button style={C.btn} className="gbtn" onClick={()=>setChat(true)}>
            💬 Написать продавцу
          </button>
          {l.showPhone!=="show"&&<p style={{...C.contactHint,marginTop:8}}>🔒 Номер продавца скрыт — общайтесь в чате</p>}
        </div>
      </div>
    </Shell>
  );
  ...
  return (
    <Shell>
      ...
      <div style={C.detailBody}>
        ...
        <div style={C.contactBox}>
          {l.showPhone==="show" && (
            <a href={`tel:+${l.sellerPhone}`} style={{...C.callBtn,marginBottom:12}}>📞 Позвонить продавцу</a>
          )}
          <div style={C.contactForm}>
            <input
              ...
              onKeyDown={e=>e.key==="Enter"&&name.trim()&&setChat(true)}
            />
            <button
              style={{...C.btn,opacity:name.trim()?1:0.4}}
              className="gbtn"
              onClick={()=>{ if(name.trim()) setChat(true); }}
            >
              💬 Написать продавцу
            </button>
            <p style={C.contactHint}>Введите имя и начните чат — продавец ответит вам</p>
          </div>
        </div>
      </div>
    </Shell>
...
function SellerView({back,onPublish}) {
  ...
  const validate=()=>{
    ...
    setErr(e);
    if(Object.keys(e).length){const first=["photo","price","city","sellerName","sellerPhone"].find(k=>e[k]);refs[first]?.current?.scrollIntoView({behavior:"smooth",block:"center"});}
  ...
  return (
    <Shell>
      ...
      <div style={C.progBar}>{[1,2,3].map(i=><div key={i} style={{...C.progSeg,background:step>=i?"linear-gradient(90deg,#f43f5e,#fb923c)":"rgba(255,255,255,0.1)"}}/>)}</div>
      <div style={{...C.page,opacity:vis?1:0,transition:"opacity 0.4s",paddingBottom:100}}>
        {step===1&&<>
          <div ref={refs.photo} style={{...C.photoZone,borderColor:err.photo?"#f87171":"rgba(255,255,255,0.1)"}} onClick={()=>fileRef.current.click()}>
            {photo?<img src={photo} style={{width:"100%",height:"100%",objectFit:"cover",borderRadius:16}} alt=""/>
              :<div style={C.photoEmpty}><div style={C.photoIc}><svg width="28" height="28" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M17 8l-5-5-5 5M12 3v12"/></svg></div><span style={C.photoLbl}>Добавить фото</span><span style={C.photoHint}>JPG, PNG • до 10 МБ</span></div>}
          </div>
          ...
          <FormCard>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {FRESH.map((fr,i)=><button key={i} style={{...C.freshOpt,borderColor:form.freshness===i?fr.color:"rgba(255,255,255,0.08)",background:form.freshness===i?fr.color+"14":"rgba(255,255,255,0.02)",color:form.freshness===i?fr.color:"rgba(255,255,255,0.45)"}} onClick={()=>set("freshness",i)}><div style={{...C.freshDot,background:fr.color,opacity:form.freshness===i?1:0.4}}/>{fr.label}{form.freshness===i&&<span style={{marginLeft:"auto"}}>✓</span>}</button>)}
            </div>
            <div style={{height:14}}/>
          ...
          <FormCard>
            ...
            <div ref={refs.sellerName}><input style={{...C.inp,borderColor:err.sellerName?"#f87171":undefined,marginBottom:10}} placeholder="Ваше имя *" value={form.sellerName} onChange={e=>set("sellerName",e.target.value)}/></div>
            <div ref={refs.sellerPhone}><input style={{...C.inp,borderColor:err.sellerPhone?"#f87171":undefined}} placeholder="Телефон * (+7 700 ...)" value={form.sellerPhone} onChange={e=>set("sellerPhone",e.target.value)}/></div>
            <div style={{height:14}}/>
            <Label>Показывать номер покупателям?</Label>
            <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:4}}>
              {[{v:"hidden",ic:"🔒",t:"Скрыть номер",s:"Покупатель пишет в чат — вы отвечаете сами"},{v:"show",ic:"📞",t:"Показать номер",s:"Номер виден всем покупателям в объявлении"}].map(o=>(
                <button key={o.v} style={{...C.radioOpt,borderColor:form.showPhone===o.v?"rgba(251,191,36,0.5)":"rgba(255,255,255,0.07)",background:form.showPhone===o.v?"rgba(251,191,36,0.07)":"rgba(255,255,255,0.02)"}} onClick={()=>set("showPhone",o.v)}>
                  <div style={{...C.radioCircle,borderColor:form.showPhone===o.v?"#fbbf24":"rgba(255,255,255,0.2)"}}>{form.showPhone===o.v&&<div style={{...C.radioDot,background:"#fbbf24"}}/>}</div>
                  <div><div style={{fontSize:13,fontWeight:600,color:form.showPhone===o.v?"#fff":"rgba(255,255,255,0.5)"}}>{o.ic} {o.t}</div><div style={{fontSize:11,color:"rgba(255,255,255,0.25)",marginTop:2}}>{o.s}</div></div>
                </button>
              ))}
            </div>
          </FormCard>
...
function PreviewCard({form,photo}) {
  ...
  return (
    <div style={C.prevCard}>
      <div style={{...C.cardImg,height:220,borderRadius:"16px 16px 0 0",background:photo?"none":"linear-gradient(135deg,#fce4ec,#f8bbd0)"}}>
        {photo?<img src={photo} style={{width:"100%",height:"100%",objectFit:"cover"}} alt=""/>:<span style={C.cardEmoji}>💐</span>}
  ...
  return (
    <div style={{animation:"fadeUp .4s ease both"}}>
      ...
      <FormCard>
        ...
        <button style={C.kaspiBtn} onClick={()=>window.open(`https://kaspi.kz/transfers/p2p?phone=${OWNER}`,"_blank")}>
          ...
          <span style={{flex:1,textAlign:"left"}}><span style={{display:"block",fontWeight:700,fontSize:14}}>Открыть Kaspi</span><span style={{display:"block",fontSize:11,color:"rgba(255,255,255,0.4)",marginTop:1}}>Перевод → {OWNER}</span></span>
      ...
      <p style={{textAlign:"center",fontSize:11,color:"rgba(255,255,255,0.2)",marginTop:14,lineHeight:1.6}}>После перевода нажмите кнопку — мы проверим и опубликуем в течение 30 минут</p>
    </div>
  );
}
    ...
    <Shell>
      <div style={C.centerBox}>
        ...
        <p style={{color:"rgba(255,255,255,0.4)",fontSize:14,lineHeight:1.7,textAlign:"center"}}>
          Переведите 500 ₸ на номер<br/><strong style={{color:"#4ade80"}}>{OWNER}</strong><br/>
...
function AdminPanel({pending,listings,approve,reject,removeL,back}) {
  ...
  return (
    <Shell>
      ...
      <div style={C.page}>
        <div style={C.tabs}>
          <button style={{...C.tab,...(tab==="pending"?C.tabOn:{})}} onClick={()=>setTab("pending")}>Заявки {pending.length>0&&<span style={C.badge}>{pending.length}</span>}</button>
          <button style={{...C.tab,...(tab==="pub"?C.tabOn:{})}} onClick={()=>setTab("pub")}>Опубликовано <span style={{color:"rgba(255,255,255,0.3)"}}>{listings.length}</span></button>
        </div>
          ...
          :pending.map(l=>(
            <div key={l.id} style={C.adminCard}>
              <div style={C.adminTop}>
                <div style={{...C.adminThumb,background:"linear-gradient(135deg,#fce4ec,#f8bbd0)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,overflow:"hidden"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:12,color:"rgba(255,255,255,0.3)",marginTop:2}}>📍 {l.city} · {l.at}</div>
                </div>
              </div>
              <div style={{background:"rgba(251,191,36,0.06)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:12,padding:"12px 14px"}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  {[["Имя",l.sellerName],["Телефон",l.sellerPhone],["Свежесть",fr(l.freshness).label]].map(([k,v])=>(
                    <div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>{k}</span>
                      {k==="Телефон"
                        ?<a href={`tel:${v}`} style={{fontSize:14,color:"#4ade80",fontWeight:700,textDecoration:"none"}}>📞 {v}</a>
                        :<span style={{fontSize:13,color:"#fff",fontWeight:500}}>{v}</span>}
                    </div>
                  ))}
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>Номер виден?</span>
        ...
        {tab==="pub"&&(listings.length===0
          ?<div style={C.empty}>💐<br/>Нет объявлений</div>
          :listings.map(l=>(
            <div key={l.id} style={{...C.adminCard,padding:"14px 16px"}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                <div style={{width:48,height:48,borderRadius:10,background:"linear-gradient(135deg,#fce4ec,#f8bbd0)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,overflow:"hidden"}}>
                ...
                <button style={{background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.2)",borderRadius:8,padding:"6px 10px",color:"#f87171",fontSize:12}} onClick={()=>removeL(l.id)}>Удалить</button>
              </div>
            </div>
          ))
        )}
...
const CSS=`
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .card:hover{transform:translateY(-4px);box-shadow:0 20px 60px rgba(0,0,0,.5)!important;}
  .gbtn:hover{transform:translateY(-2px);filter:brightness(1.1);}.gbtn:active{transform:translateY(0);}
`;
...
const C={
  ...
  home:{position:"relative",zIndex:1,maxWidth:480,margin:"0 auto",padding:"0 20px 60px",display:"flex",flexDirection:"column",alignItems:"stretch"},
  ...
  modeCard:{position:"relative",overflow:"hidden",padding:"24px 20px",borderRadius:20,border:"1px solid rgba(255,255,255,0.07)",background:"rgba(255,255,255,0.03)",backdropFilter:"blur(20px)",display:"flex",flexDirection:"column",alignItems:"flex-start",gap:6,transition:"transform .2s",cursor:"pointer",textAlign:"left"},
  ...
  statIc:{fontSize:18},statV:{fontSize:18,fontWeight:700,color:"#fff"},
  ...
  count:{fontSize:12,color:"rgba(255,255,255,0.25)",marginBottom:14},
  ...
  card:{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:18,overflow:"hidden",cursor:"pointer",transition:"transform .2s,box-shadow .2s"},
  ...
  cardCity:{fontSize:10,color:"rgba(255,255,255,0.3)",marginBottom:4},
  ...
  detailTitle:{fontSize:26,fontWeight:700,fontFamily:"'Fraunces',serif",color:"#fff",lineHeight:1.2,marginBottom:14},
  ...
  stickyBar:{position:"fixed",bottom:0,left:0,right:0,zIndex:100,padding:"12px 16px",background:"rgba(12,12,16,0.95)",borderTop:"1px solid rgba(255,255,255,0.06)",backdropFilter:"blur(20px)",display:"flex",gap:10},
  ...
  kaspiBtn:{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderRadius:14,border:"1px solid rgba(241,70,53,0.25)",background:"rgba(241,70,53,0.06)",color:"#fff",marginBottom:14,fontFamily:"Inter,sans-serif"},
  ...
  adminThumb:{width:72,height:72,borderRadius:12,flexShrink:0},
